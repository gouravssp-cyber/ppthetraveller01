...
class FilterSectionProducts extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $query = Product::where('is_active', true)
            ->whereHas('sections', function ($q) {
                $q->where('sections.id', $this->sectionId);
            })
            ->with('category');
        ...
        // Get filter options based on products in this section
        $categories = Category::where('is_active', true)
            ->whereHas('products', function ($q) {
                $q->whereHas('sections', function ($sq) {
                    $sq->where('sections.id', $this->sectionId);
                });
            })
            ->orderBy('name')
            ->get();
        ...
        $priceRange = [
            'min' => Product::where('is_active', true)
                ->whereHas('sections', function ($q) {
                    $q->where('sections.id', $this->sectionId);
                })
                ->min('min_price') ?? 0,
            'max' => Product::where('is_active', true)
                ->whereHas('sections', function ($q) {
                    $q->where('sections.id', $this->sectionId);
                })
                ->max('max_price') ?? 10000,
        ...
        return view('livewire.filter-section-products', [
            'section' => $section,
            'products' => $products,
